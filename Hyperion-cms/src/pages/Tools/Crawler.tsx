...
import { Bot, Play, RefreshCw, ExternalLink, Plus, Settings, X, Trash2, Check, AlertCircle, Loader2, Code } from 'lucide-react';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { crawlerApi, type CrawlerSource } from '../../api/crawler';
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-gray-50/50">
                        ...
                        <button className="flex items-center px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg text-xs font-bold hover:bg-gray-50 transition-all shadow-sm active:scale-95">
                            <Play className="h-3.5 w-3.5 mr-2 text-green-600" /> Run All
                        </button>
                    </div>
                ...
                {/* Console / Log area */}
                <div className="bg-slate-900 rounded-2xl shadow-2xl p-6 relative overflow-hidden group border border-slate-800">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            <div className="h-2 w-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_8px_rgba(34,197,94,0.5)]"></div>
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Aggregation Engine Status: Normal</span>
                        </div>
                        <span className="text-[10px] font-mono text-gray-600">v2.1.0-stable</span>
                    </div>
                    <div className="space-y-1 font-mono text-xs text-gray-400 leading-relaxed max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-800">
                        <p className="text-gray-600">[{new Date().toLocaleTimeString()}] System bootstrap complete...</p>
                        <p className="text-blue-400">[{new Date().toLocaleTimeString()}] Polling worker active (interval: 300s)</p>
                        <p>&gt; Connection established to data relay...</p>
                        <p className="text-green-500/80">&gt; Healthy: All configured sinks operational</p>
                    </div>
                </div>
            </div>
