...
import { Loader2, ArrowLeft, Save, Trash2, Image as ImageIcon, Layout, Type, Link, Video, FileText, Search, X, ArrowUp, ArrowDown, Eye } from 'lucide-react';
...
export function StorylineEditor() {
    ...
    const saveMutation = useMutation({
        mutationFn: (data: Storyline) => {
            const payload = { ...data, contentsJson: JSON.stringify(blocks) };
                                        ...
                                        <div className="border-2 border-dashed border-gray-200 rounded-lg p-6 bg-gray-50 flex flex-col items-center justify-center">
                                            {block.settings?.url ? (
                                                <div className="relative w-full">
                                                    {block.type === 'image' ? (
                                                        <img src={`http://localhost:8081${block.settings.url}`} alt="Block media" className="max-h-[300px] rounded-lg mx-auto object-contain" />
                                                    ) : (
                                                        <video src={`http://localhost:8081${block.settings.url}`} controls className="max-h-[300px] rounded-lg mx-auto w-full" />
                                                    )}
                                            ...
                                            {!block.content ? (
                                                ...
                                                <div className="flex items-center justify-between bg-white border border-gray-200 p-3 rounded-lg">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="text-sm font-semibold text-gray-900">{block.settings?.title || 'Linked Article'}</div>
                                                            <div className="text-xs text-gray-500">ID: {block.content}</div>
                                                        </div>
                                                    </div>
                                                    <button
